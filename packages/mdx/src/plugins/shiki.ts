...
export interface Options {
   ...
   */
  highlight?: (code: string, lang: string, props: string[]) => string | Promise<string>;
}
  ...
  return {
    ...
    async walkTokens(token) {
      ...
      const htmlText = container
        ? container.replace('%l', String(lang).toUpperCase()).replace('%s', highlightedText).replace('%t', text)
        : highlightedText;
