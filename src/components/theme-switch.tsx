...
/**
 * ThemeSwitchSkeleton Component
 * A lightweight placeholder that matches the button size.
 * Uses CSS to show the appropriate icon based on theme class (no JS needed).
 */
const ThemeSwitchSkeleton = () => (
  <Button variant="ghost" size="icon" aria-label="Theme switch button" disabled>
...
/**
 * ThemeSwitchAnimated Component
 * The animated theme switch with framer-motion (client-side only).
 */
const ThemeSwitchAnimated = () => {
 ...
 * Uses a skeleton on SSR that shows the correct icon via CSS, then swaps to
 * the animated version on client.
 *
 * @returns {React.ReactNode} The rendered ThemeSwitch component.
 */
const ThemeSwitch = (): React.ReactNode => {
  // useSyncExternalStore with getServerSnapshot returning false ensures
  // no hydration mismatch - server always renders skeleton, client swaps after hydration
